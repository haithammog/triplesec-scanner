...
| Webhook | vuln-scanner.py | 933–945 | post_webhook |
...
| Config template | config.toml | 1–135 | All keys documented, safe defaults |
...
5. Reports (HTML + PDF) and chart generated; results POSTed to n8n webhook URL from config

## Entry Points
| Type | Location | Description |
|---|---|---|
| CLI | vuln-scanner.py:main() | `python3 vuln-scanner.py <network> <email> [flags]` |
| API | vuln-scanner.py GET /health | Health check |
| API | vuln-scanner.py POST /scan | Trigger scan (202 Accepted, runs in daemon thread) |
...
- Never hardcode any URL, path, threshold, or timeout — all must come from config
